...
const Pagination = ({ setIsLoading }: IPaginationProps) => {
  ...
  const { total, paginate, page, setTotal } = usePagination({
    PER_PAGE,
    totalPages,
    searchCheckbox,
  });
  ...
  const handlePagination = (page: number) => {
    ...
    dispatch(
      getProductsAction(
        page,
        searchCheckbox.active,
        searchCheckbox.promo,
  ...
  return (
    <PaginationWrapper>
      <button onClick={() => handlePagination(1)} disabled={1 === page}>
        First
      </button>
      ...
      <button onClick={() => handlePagination(total)} disabled={total === page}>
        Last
      </button>
    </PaginationWrapper>
  );
